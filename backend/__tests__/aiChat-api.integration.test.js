...
describe('POST /api/ai-chat/message', () => {
  ...
  describe('成功訊息流程', () => {
    it('通過驗證與 Rate_Limiter 後呼叫 chatAssistantService.handleMessage，回傳 { session, replyText, replyMeta }', async () => {
      ...
      expect(res.body.replyMeta).toEqual({ blocked: false, reason: null, topicType: null, topicRequired: true, options: null, savedAddresses: null });
  ...
  describe('輸入驗證阻擋（Requirement 1.6, 1.7, 9.5, 9.6）', () => {
    it('handleMessage 判定輸入被阻擋時，route 應原樣反映 blocked:true 與 reason', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.replyMeta).toEqual({ blocked: true, reason: 'TOO_LONG', topicType: null, topicRequired: true, options: null, savedAddresses: null });
