...
const trashPhotoInput = ref<HTMLInputElement | null>(null)
const trashPhotos = ref<string[]>([])
...
const trashMessages = ref<TrashMessage[]>([])
const isTrashAiThinking = ref(false)

function triggerTrashPhotoUpload() {
  trashPhotoInput.value?.click()
}

function handleTrashPhotoChange(event: Event) {
  const input = event.target as HTMLInputElement
  if (!input.files) return
  for (const file of Array.from(input.files)) {
    const reader = new FileReader()
    reader.onload = (e) => {
      if (e.target?.result) {
        trashPhotos.value.push(e.target.result as string)
      }
    }
    reader.readAsDataURL(file)
  }
...
function removeTrashPhoto(index: number) {
  trashPhotos.value.splice(index, 1)
}

function submitTrashQuery() {
  if (!trashQuery.value.trim() && trashPhotos.value.length === 0) return
  const userText = trashQuery.value.trim() || '(已上傳照片)'
  trashMessages.value.push({ id: `msg-${Date.now()}`, role: 'user', text: userText })
  trashQuery.value = ''
  trashPhotos.value = []

  // Simulate AI thinking
  isTrashAiThinking.value = true
  setTimeout(() => {
    isTrashAiThinking.value = false
    // Mock: if user mentioned 紙便當盒, ask follow-up
    if (userText.includes('紙便當') || userText.includes('便當盒')) {
      trashMessages.value.push({
        id: `msg-${Date.now()}`,
        role: 'ai',
        text: '💡 請問這個紙便當盒內層是否有厚塑膠膜？或者是否油污無法洗淨？',
        quickReplies: ['已洗乾淨', '嚴重油污 (垃圾)'],
      })
    } else if (userText.includes('電池') || userText.includes('舊電池')) {
      trashMessages.value.push({
        id: `msg-${Date.now()}`,
        role: 'ai',
        text: '🔋 舊電池屬於「有害垃圾」，請投入超商或區公所設置的廢電池回收桶，切勿混入一般垃圾。',
      })
    } else if (userText.includes('已洗乾淨')) {
      trashMessages.value.push({
        id: `msg-${Date.now()}`,
        role: 'ai',
        text: '♻️ 紙類 (需清洗乾淨並扁平堆疊) — 請投入資源回收的「紙類」分類。',
      })
    } else if (userText.includes('嚴重油污')) {
      trashMessages.value.push({
        id: `msg-${Date.now()}`,
        role: 'ai',
        text: '🗑️ 嚴重油污無法清洗的紙便當盒歸類為「一般垃圾」，請直接丟入垃圾袋。',
      })
    } else {
      trashMessages.value.push({
        id: `msg-${Date.now()}`,
        role: 'ai',
        text: '♻️ 根據您的描述，此物品屬於「資源回收 - 紙類/塑膠類」，請清洗後投入回收分類。',
      })
    }
  }, 1200)
}
...
const truckStops = ref<TruckStop[]>([
  {
    id: 'truck-1',
    name: '忠孝東路五段 210 號清運點',
    distance: '150m',
    arriveTime: '19:30',
    minutesLeft: 5,
    categories: ['一般垃圾', '資源回收', '廚餘'],
    urgent: true,
  },
  {
    id: 'truck-2',
    ...
    arriveTime: '20:15',
    minutesLeft: 50,
    categories: ['一般垃圾', '資源回收'],
    urgent: false,
  },
])

function handleSetReminder(stopId: string) { console.log('開啟到達提醒:', stopId) }
function handleNavigate(stopId: string) { console.log('開啟導航:', stopId) }
  ...
  <div class="housing-module">
    <main class="housing-page" role="main">
      ...
      <section v-if="activeTab === 'trash'" class="tab-content" aria-label="垃圾分類">
        ...
        <!-- ♻️ AI 垃圾分類助手 -->
        <div class="trash-ai-card">
          <h3 class="trash-ai-card__title">♻️ AI 垃圾分類助手</h3>

          <!-- 對話歷史 -->
          <div v-if="trashMessages.length > 0" class="trash-chat">
            <div
              v-for="msg in trashMessages"
              :key="msg.id"
              :class="['trash-chat__bubble', msg.role === 'user' ? 'trash-chat__bubble--user' : 'trash-chat__bubble--ai']"
            >
              <p class="trash-chat__text">{{ msg.text }}</p>
              <!-- 快捷回覆按鈕 -->
              <div v-if="msg.quickReplies && msg.quickReplies.length > 0" class="trash-chat__quick-replies">
                <button
            ...
            <div v-if="isTrashAiThinking" class="trash-chat__bubble trash-chat__bubble--ai">
              <p class="trash-chat__text trash-chat__thinking">🤔 分析中...</p>
            </div>
          </div>

          <!-- 照片上傳預覽 -->
          <div v-if="trashPhotos.length > 0" class="photo-upload__previews">
            <div v-for="(photo, idx) in trashPhotos" :key="idx" class="photo-thumb">
              <img :src="photo" alt="垃圾照片預覽" class="photo-thumb__img">
              <button class="photo-thumb__remove" type="button" @click="removeTrashPhoto(idx)">✕</button>
            </div>
          </div>

          <!-- 輸入區域 -->
          <div class="trash-ai-input">
            <button class="trash-ai-input__photo-btn" type="button" @click="triggerTrashPhotoUpload">
              📷
            </button>
            <input
              ref="trashPhotoInput"
              ...
              multiple
              class="sr-only"
              @change="handleTrashPhotoChange"
            >
            <input
              v-model="trashQuery"
              type="text"
              class="trash-ai-input__text"
              placeholder="輸入垃圾名稱，例如：紙便當盒、舊電池、保麗龍..."
              @keyup.enter="submitTrashQuery"
            >
            <button class="trash-ai-input__submit" type="button" @click="submitTrashQuery">
              � 查詢分類
            </button>
          </div>
        </div>
        ...
        <!-- 🚚 即時垃圾車定位與倒數 -->
        <div class="truck-location-card">
          <p class="truck-location-card__status">📍 已定位：台北市信義區 (距離目前位置預測)</p>
        </div>

        <div v-for="stop in truckStops" :key="stop.id" :class="['truck-card', { 'truck-card--urgent': stop.urgent }]">
          <div class="truck-card__header">
            <h4 class="truck-card__name">{{ stop.name }}</h4>
            <span class="truck-card__distance">📍 距離 {{ stop.distance }}</span>
          </div>
          <div class="truck-card__time">
            <span>{{ stop.arriveTime }} 到達</span>
            <span :class="['truck-card__countdown', { 'truck-card__countdown--urgent': stop.urgent }]">
              還剩 {{ stop.minutesLeft }} 分鐘
            </span>
          </div>
          <div class="truck-card__categories">
            <span v-for="cat in stop.categories" :key="cat" class="truck-card__category-badge">
              {{ cat }}
            </span>
          </div>
          <div class="truck-card__actions">
            <button class="btn btn--outline" type="button" @click="handleSetReminder(stop.id)">
              🔔 開啟到達提醒 (前 10 分鐘)
            </button>
            <button class="btn btn--green" type="button" @click="handleNavigate(stop.id)">
              🧭 開啟導航前往
            </button>
          </div>
        </div>
